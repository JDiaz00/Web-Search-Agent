...
class AgentRequest(BaseModel):
    ...
    query: str = Field(
        ..., description="The user query to process with the agent", max_length=2000
    )
    session_id: Optional[str] = Field(
        None, description="Optional session ID for tracking conversations"
    )
...
class AgentResponse(BaseModel):
    ...
    steps: List[str] = Field(
        default_factory=list, description="The steps taken by the agent"
    )
